...
Hight_plot hight_plot_create(double (*hight_func)(double, double), double (*hight_func_deriv)(double, double, char), float min_e1_val, float max_e1_val, float min_e2_val, float max_e2_val, float min_func_val, float max_func_val, float func_offset, float min_e1_pos, float max_e1_pos, float min_e2_pos, float max_e2_pos, int num_sample_e1, int num_sample_e2, char *plane, float third_direction_grid_position)
{
    ...
    for (float e1_val = min_e1_pos, e1_val_func = hight_plot.min_e1; e1_val < max_e1_pos - de1; e1_val += de1, e1_val_func += hight_plot.de1) {
        for (float e2_val = min_e2_pos, e2_val_func = hight_plot.min_e2; e2_val < max_e2_pos - de2; e2_val += de2, e2_val_func += hight_plot.de2) {
            ...
            ae_point_normalize(quad.normals[3]);
            ...
            ae_point_normalize(quad.normals[2]);
            ...
            ae_point_normalize(quad.normals[1]);
            ...
            ae_point_normalize(quad.normals[0]);
            #else
...
void setup(game_state_t *game_state)
{
    ...
    // hight_plot = hight_plot_create(hight_func_inv, hight_func_inv_deriv, -10 , 10, -10, 10, -1, 1, 0.1, -2, 2, -2, 2, 100, 100, "XZ", 1);
    // hight_plot = hight_plot_create(hight_func_trig, hight_func_trig_deriv, 0 , 6*PI, 0 , 6*PI, -1, 1, 1.1, -2, 2, -2, 2, 100, 100, "XZ", 1);
    hight_plot = hight_plot_create(hight_func_exp, hight_func_exp_deriv, -1, 1   , -1, 1   , 0 , 3, 0.1, -0.5, 0.5, -0.5, 0.5, 30, 30, "XZ", 1);
...
void render(game_state_t *game_state)
{
    ...
    adl_quad_mesh_fill_interpolate_normal(game_state->window_pixels_mat, game_state->inv_z_buffer_mat, hight_plot.proj_quads, 0xFF55FF55, ADL_DEFAULT_OFFSET_ZOOM);
